sap.ui.define([
    ...
    function (Controller) {
        ...
        return Controller.extend("com.techm.sustainabilityui.controller.TrackerForm", {
            onInit: function () {
                that = this;
                ...
                //Direct Energy - Options - DEOptions
            ...
            onUpdateFValue: function () {
                ...
                var OData = this.getOwnerComponent().getModel("dataModel");
                // var OData = new sap.ui.model.odata.v2.ODataModel("/v2/catalog", true);
                OData.read("/Environment_Report_Temp", {
                    //   filters: aFilters,
                    success: function (data, oResponse) {
                        // oBusyDialog.close();
                        ListModel.setData(data);
                        ...
                        for (var i = 0; i < data.results.length; i++) {
                            if (data.results[i].KPI === "Refrigerant (R32)") {
                                var json2 = {
                                    "Year": data.results[i].Year,
                                    "Month": data.results[i].Month,
                                    "Location": data.results[i].Location,
                                    "Type": data.results[i].Type,                  
                                   "SubType": data.results[i].SubType,           
                                    "KPI": data.results[i].KPI,            
                                   "Value": parseFloat(data.results[i].Value),                                    
                                    "Unit": data.results[i].Unit,
                                    "Cost": data.results[i].Cost,
                                    "Currency": data.results[i].Currency,
                                    "Quality": data.results[i].Quality,
                                    "Comment": data.results[i].Comment,
                                    "Owner": data.results[i].Owner,
                                    "Approver": data.results[i].Approver,
                                    "Division": data.results[i].Division,
                                    "Value2": data.results[i].Value2,
                                    "Unit2": data.results[i].Unit2,
                                    "Distance": data.results[i].Distance,
                                    "Weight": data.results[i].Weight,
                                    "Scope": data.results[i].Scope,
                                    "Associates": data.results[i].Associates,
                                    "Sno": data.results[i].Sno,
                               ...
                               OData.create('/Environment_Report', json2, null, function () {
            ...
            onPressSubmit_V1: function (event) {
                var that = this;

                var jsonSustain =
                {
                    "Year": that.getView().byId("box2").getSelectedKey(),
                    "Month": that.getView().byId("directEnergy").getItems()[0].getCells()[0].mProperties.title,
                    "Value": that.getView().byId("directEnergy").getItems()[0].getCells()[1].mProperties.value,
                    "Unit": "gl",
                    "Cost": that.getView().byId("directEnergy").getItems()[0].getCells()[3].mProperties.value,
                    "Currency": "USD",
                    "Quality": "OK",
                    "Comment": "NA",
                    "Type": "Direct Energy",
                    "SubType": that.getView().byId("deOptions").getSelectedKey(),
                    "Owner": that.getView().byId("directEnergy").getItems()[0].getCells()[7].mProperties.text,
                    "Approver": that.getView().byId("directEnergy").getItems()[0].getCells()[8].mProperties.text,
            ...
            handleUploadPress: function () {
                ...
                var options = {
                    ...
                    "documentType": "custom",
                    "enrichment": {},
                    "schemaId": "6b544b34-263d-4e23-9fe4-53be6e58b54b",
                    "templateId": "a64e9813-0a4b-4e00-9d37-14ad232dd7ba"
                };
                ...
                oFormData.append("options",
                    "{\"clientId\": \"default\",\"documentType\": \"invoice\",\"enrichment\": {},\"schemaId\": \"6b544b34-263d-4e23-9fe4-53be6e58b54b\",\"templateId\": \"a64e9813-0a4b-4e00-9d37-14ad232dd7ba\"}"
                );
                ...
                jQuery.ajax({
                    url: "https://3c0603actrial.authentication.us10.hana.ondemand.com/oauth/token?grant_type=client_credentials",
                    method: "GET",
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        "Authorization": 'Basic c2ItZGQ2MmE5YzAtZTYzZS00ZTgwLTg4MTQtYjM4OGVlMDIzYzE3IWIxMTA4Mzd8ZG94LXhzdWFhLXN0ZC10cmlhbCFiMTA4NDQ6Y2RkMjM0NmUtNjRiMy00NTAwLThiNTgtMDJlNDNiY2Q5ZmJmJHh1Rng1MFNuYm1qcjY1cnJuU1FlVlc4VDVQUnpWY1kwQTk3aEhXRm1DaVk9'
                    },
