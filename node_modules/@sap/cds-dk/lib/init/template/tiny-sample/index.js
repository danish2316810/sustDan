...
module.exports = class TinySampleTemplate extends require('../templateBase') {
    ...
    async _addJavaSamples() {
        if (!cds.cli.options.add.has(JAVA)) {
            const mvnAddSamplesArgs = mvnArchetypes.getAddSamplesCmdArgs()
            try {
                await commandUtil.spawnCommand('mvn', mvnAddSamplesArgs, { cwd: cds.root })
                const mvnAddTestsArgs = mvnArchetypes.getAddTestsCmdArgs()
    ...
    async _addNodejsSamples() {
        ...
        await copyFiles(join(__dirname, 'files', 'nodejs', 'db'), db, {}, cds.cli.options.force)
        await copyFiles(join(__dirname, 'files', 'nodejs', 'srv'), srv, { dbFolder: db.replace(/[\\/]+$/, '') }, cds.cli.options.force)
    }
}
